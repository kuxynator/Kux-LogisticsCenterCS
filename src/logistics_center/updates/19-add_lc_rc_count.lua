...
function add_lc_rc_count()
    if global.global_data_version < v then
        ...
        for k, surface in pairs(game.surfaces) do
			...
			local ccspp = surface.find_entities_filtered {name = names.collecter_chest_1_1.."-pp"} --TODO KUX
            for k1, v in pairs(ccs) do
                re_scan_add_cc(v)
			end
			local ccss = surface.find_entities_filtered {name = names.collecter_chest_1_1.."-pp"} --TODO KUX
            for k1, v in pairs(ccs) do
                re_scan_add_cc(v)
            end
			...
			local rcsb = surface.find_entities_filtered {name = names.requester_chest_1_1.."b"} --TODO KUX
            for k1, v in pairs(rcs) do
                re_scan_add_rc(v)
			end
